...
class VarNetSample(NamedTuple):
    ...
    t2_gt: torch.Tensor
    pd_gt: torch.Tensor
    # added functio XML handler
    def parse_xml_physics_params(xml_str: str) -> Tuple[float, float, float]:
        """
        Extract TR, TE, and Alpha from ISMRMRD XML string.
        Handles both namespaced (standard fastMRI) and non-namespaced (synthetic) tags.
        """
        try:
            ...
            def get_physics_value(tag_name):
                # 1. Try finding with namespace prefix 'ns'
                # 2. Try finding without any prefix
                namespaces = {'ns': 'http://www.ismrm.org/ISMRMRD'}
                
                # Search using XPath that finds the tag anywhere in the tree
                element = root.find(f".//ns:{tag_name}", namespaces)
                if element is None:
                    element = root.find(f".//{tag_name}")
                
                return float(element.text) if element is not None and element.text else 0.0

            tr = get_physics_value('TR')
            te = get_physics_value('TE')
            # Check for flipAngle_deg or TI if needed
            alpha = get_physics_value('flipAngle_deg')
            
            return tr, te, alpha
        except Exception:
            # If parsing fails, return zeros to avoid crashing the training loop
            return 0.0, 0.0, 0.0
...
class VarNetDataTransform:
    ...
    def __call__(
        ...
        fname: str,
        slice_num: int,
        t1_map: Optional[np.ndarray] = None,
        t2_map: Optional[np.ndarray] = None,
        ...
        def process_tissue_map(m):
            if m is not None:
                # הפיכה לטנזור והוספת מימד ערוץ [1, H, W]
                m_torch = torch.from_numpy(m).unsqueeze(0)
                # ביצוע Center Crop כדי שיתאים ל-target
                return center_crop(m_torch, crop_size)
            return torch.tensor(0.0)
        
        t1_gt = process_tissue_map(t1_map)
        t2_gt = process_tissue_map(t2_map)
        pd_gt = process_tissue_map(pd_map)

        # [added] --- Start of Physics Extraction ---
        tr, te, alpha = 0.0, 0.0, 0.0
        
        # עדיפות לערכים שהגיעו ישירות מהפונקציה
        if tr_val > 0:
            tr, te, alpha = tr_val, te_val, alpha_val

        elif "ismrmrd_header" in attrs:
            try:
                xml_str = attrs["ismrmrd_header"]
                if isinstance(xml_str, (bytes, np.bytes_)):
                    xml_str = xml_str.decode("utf-8")
                
                # Use a very permissive parser logic
                root = ET.fromstring(xml_str)
                
                # Search for tags regardless of depth or namespace
                for element in root.iter():
                    tag_name = element.tag.split('}')[-1] # Remove namespace if exists
                    if tag_name == 'TR':
                        tr = float(element.text)
                    elif tag_name == 'TE':
                        te = float(element.text)
                    elif tag_name == 'flipAngle_deg':
                        alpha = float(element.text)
            except Exception as e:
                print('[DEBUG] : im in transforms script inside ismrmrd header -EXPCEPION')
                # English: Error parsing XML, will try fallback
                pass

        # Fallback to direct attributes if XML parsing failed to find values
        if tr == 0.0 and "tr" in attrs:
            print('[DEBUG] : im in transforms script inside ismrmrd header -if tr == 0.0')
            tr = float(attrs.get("tr", 0.0))
            te = float(attrs.get("te", 0.0))
            alpha = float(attrs.get("alpha", 0.0))

        # [added]--- End of Physics Extraction ---
