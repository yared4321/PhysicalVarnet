...
class SliceDataset(torch.utils.data.Dataset):
    ...
    def _retrieve_metadata(self, fname):
        with h5py.File(fname, "r") as hf:
            ...
            header_data = hf["ismrmrd_header"][()]
        
            # Ensure the data is in a format etree can handle (str or bytes)
            # Some h5py versions return a numpy object that needs .item() or decoding
            if hasattr(header_data, 'item'):
            ...
            et_root = etree.fromstring(header_data)
    ...
    def __getitem__(self, i: int):
        ...
        with h5py.File(fname, "r") as hf:
            ...
            t1_map = hf["t1_map"][dataslice] if "t1_map" in hf else None
            t2_map = hf["t2_map"][dataslice] if "t2_map" in hf else None
            pd_map = hf["pd_map"][dataslice] if "pd_map" in hf else None

            attrs = dict(hf.attrs)
            # [added] START
            attrs["ismrmrd_header"] = hf["ismrmrd_header"][()]
            # [added] END
            ...
            alpha = attrs.get("alpha", 0.0)
             # חילוץ הלייבל
            contrast_label = self.get_contrast_label(tr)

            if self.transform is None:
                sample = (kspace, mask, target, attrs, fname.name, dataslice,t1_map, t2_map, pd_map, tr, te, alpha)
            else:
                sample = self.transform(kspace, mask, target, attrs, fname.name, dataslice, t1_map, t2_map, pd_map, tr, te, alpha, contrast_label)

        return sample

    def get_contrast_label(self, tr_val):
        """
        Assigns a contrast label based on TR value.
        """
        if tr_val <= 1000: 
            val= 0.15
        else:
